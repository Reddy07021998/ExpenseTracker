...
# Add Expense Screen
    ...
    if expenses_df.empty:
        ...
        expense_ids = expenses_df['Expense ID'].tolist()  # Ensure this is correctly fetching all IDs
        ...
        if selected_expense_id != "Select":
            ...
            # Display fields for editing (populate them with current values)
            expense_name = st.text_input("Expense Name", value=expense_details['Expense Name'])
            amount = st.number_input("Amount", value=expense_details['Amount'], min_value=0.01, step=0.01)
            expense_date = st.date_input("Expense Date", value=pd.to_datetime(expense_details['Expense Date']))
            category_id = expense_details['Category']  # Assuming this is a category name

            # Save changes button
            if st.button("Save Changes"):
                run_async(update_expense(selected_expense_id, 
                                          st.session_state.user_id, 
                                          expense_name, 
                                          amount, 
                                          expense_date.isoformat(), 
